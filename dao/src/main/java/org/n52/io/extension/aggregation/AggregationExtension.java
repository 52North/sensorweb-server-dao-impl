...
public class AggregationExtension extends MetadataExtension<DatasetOutput< ? >> {
    ...
    public AggregationExtension(AggregationService service) {
    ...
    public Map<String, Object> getExtras(DatasetOutput<?> output, IoParameters parameters) {
        ...
        return wrapSingleIntoMap(getAggregations(parameters, output));
    }

    private AggregationOutput<AbstractValue<?>> getAggregations(IoParameters parameters, DatasetOutput< ? > output) {
        return service.getAggregations(parameters, output.getId());
    }
